...
var DoubleArray, MAX_REMOVALS_BEFORE_SWAP, ObjectStorage, RBush, SortableStack, bboxArea, bboxMargin, calcBBox, contains, createNode, distBBox, enlargedArea, extend, intersectionArea, intersects, rayBboxDistance;

MAX_REMOVALS_BEFORE_SWAP = 50;

DoubleArray = /*@__PURE__*/(function () {
  function DoubleArray(size) {
  ...
  DoubleArray.prototype.push = function push (item, item2) {
  ...
  DoubleArray.prototype.pop = function pop () {
    if (this.currentLen > 0) {
      return this.current[--this.currentLen];
    }
  };

  DoubleArray.prototype.enlarge = function enlarge () {
    this.size = Math.floor(this.size * 1.5);
    return this.current.length = this.other.length = this.size;
  };

  return DoubleArray;
}());

ObjectStorage = /*@__PURE__*/(function (DoubleArray) {
  function ObjectStorage() {
    DoubleArray.apply(this, arguments);
    this.removalsCount = 0;
  }

  if ( DoubleArray ) ObjectStorage.__proto__ = DoubleArray;
  ObjectStorage.prototype = Object.create( DoubleArray && DoubleArray.prototype );
...
SortableStack = /*@__PURE__*/(function (DoubleArray) {
  function SortableStack () {
    DoubleArray.apply(this, arguments);
  }

  if ( DoubleArray ) SortableStack.__proto__ = DoubleArray;
  SortableStack.prototype = Object.create( DoubleArray && DoubleArray.prototype );
...
RBush = /*@__PURE__*/(function () {
        ...
        if (!child._ignore) {
          if (intersects(bbox, child.bbox)) {
            if (node.leaf) {
              ...
              this._all(child, result, predicate);
            } else {
  ...
  RBush.prototype._all = function _all (node, result, predicate) {
    var child, j, len, nodesToSearch, ref;
    nodesToSearch = [];
    while (node) {
      if (node.leaf) {
        ref = node.children;
